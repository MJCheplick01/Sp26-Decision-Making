...
#moving average are considered independent, and are considered standard error: assumption, seperated out
#theory holds only if data is statinary= correlation between indepdendent variables
#properties of stationarity: constant mean and constant variance 
#co-linearity data: variance
...
```{r}
#decompose stock close price data and see pattern
...
roll_std <- rollapply(stockData$Close, width = 12, FUN = sd, fill = NA)

plot(stockData$Close, main = "Rolling Mean & Std Dev")
lines(roll_mean, col = "red", lwd = 2)
lines(roll_std, col = "green", lwd = 2)

adf_test<- adf.test(stockData$Close)
print(adf_test)

```
```{r}

###because data original is not stationary, run lagged regression
lagged_stockData <- diff(stockData$Close,lag=1)
lagged_stockData<- na.omit(lagged_stockData)
adf_test<- adf.test(lagged_stockData)
print(adf_test)
plot(lagged_stockData)
```
...
```
```{r}
acf(lagged_stockData, main = "ACF of Stockprice difference")
pacf(lagged_stockData, main = "PACF of stock price difference")
...
lines(AirPassengers, col="red", lwd=2)
head(air_boxcox)
head(AirPassengers)
```
```{r}
...
```{r}
# Box-Cox Transformation (Automatic)
lambda <- BoxCox.lambda(stockData$Close)
close_boxcox <- BoxCox(stockData$Close, lambda)
###can also use the varible recommended by Box-Box transformation of 1.21

plot(close_boxcox, main = paste("Box-Cox Transformation (lambda =", round(lambda, 2), ")"))
```
```{r}
...
predicted_travel<- forecast(model,12)
plot(predicted_travel)
predicted_travel
```
```{r}
###VARS paclage to consider exogenous variables. 
```
