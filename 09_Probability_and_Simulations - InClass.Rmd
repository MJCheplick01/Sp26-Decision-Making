...
- rnorm() #generates random numbers from a distribution (normal in this case)
- dnorm() #Heigh of the curve at the exact point of x. It is not probability, but we can interpret the density as how croded it this x with respect to the rest of the distribution. 
- pnorm() 
- qnorm() 
...
density_sample <- dnorm(sorted_water_consumption,
                        ...
                        sd=1.5) 
head(density_sample)
plot(x= sorted_water_consumption,y= density_sample, type = 'l')
...
```{r}
library(ggplot2)
ggplot(data.frame(x= human_water_consumption)) + geom_density(aes(x))
```
...
ifelse(x1 >x2, "1 litre consumption is popular=", "5 litre consumption is popular")
...
Demand in products | Probability | Cumulative Probability 
---- | ---- | ----
100 | 0.15 | 0
150 | 0.24 | 0.15
175 | 0.16 | 0.39 
...
#instead of discrete use Uniform distribution 
set.seed(1234) 
n_simulations <- 10000 
...
#add the round function to make sure the result makes sense
...
length(simulated_demand_range)
```
```{r}
head(simulated_demand,10)
head(simulated_demand_range,10)
```
